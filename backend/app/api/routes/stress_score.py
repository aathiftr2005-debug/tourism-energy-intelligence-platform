...
async def get_stress_score(
    ...
    year: int = Query(default_factory=lambda: datetime.now(timezone.utc).year, description="Year"),
    month: int = Query(default_factory=lambda: datetime.now(timezone.utc).month, ge=1, le=12, description="Month (1-12)"),
):
